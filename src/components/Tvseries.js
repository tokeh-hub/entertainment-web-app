...
const Tvseries = ({searchTerm,moviesList,len,handleBookmark}) => {
  const filtered = moviesList.filter(data=> data.category === "TV Series")
  const [tvSeries,setTv] = useState(filtered)
    ...
    <div className='tvseries'>
     ...
     <div className="search text-white">
       <h1 className="text-3xl mb-4">Found {len} results for {searchTerm}</h1> 
          ...
          {tvSeries.map((data) => {
            ...
            return (
              <div className="mr-3 mb-3">
                ...
                >
                  <button className="absolute top-1 bg-gray-600 w-10px h-10px rounded-full p-2 right-1"  onClick={()=>handleBookmark(data.title,setTv,tvSeries)}>
        ...
        {!searchTerm && <div className="tvserie text-white">
        <h1 className="text-3xl mb-4">Tv Series</h1>
        <div className="grid lg:grid-cols-4 md:grid-cols-3 grid-cols-2 gap-4">
          {tvSeries.map((data) => {
            return (
              <div className="mr-3 mb-3" >
